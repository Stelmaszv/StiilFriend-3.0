...
StartControlers.controller("MainController",['$scope','$http','Sesion','Socket','Messages','Notification','Componets','TimeConvert','$interval','$filter','Show','Chat','Mobile','Url','$timeout','UserInJason',function($scope,$http,Sesion,Socket,Messages,Notification,Componets,TimeConvert,$interval,$filter,Show,Chat,Mobile,Url,$timeout,UserInJason){
    if(Sesion.Tokken.login){
        $timeout(function(){ 
            Componets.Slids();
            MessagessFunction();
        ...
        var Actions= function(){
            CloseActiveChatsMobile=document.querySelector('.CloseActiveChatMobile');
            CloseActiveChatsMobile.addEventListener('click',function(event){
                $timeout(function () {
                    $scope.show.ActiveChatOn=false;
                }, 1);
            })
            OpenActiveChatsMobile=document.querySelector('.ActiveChatOn');
            OpenActiveChatsMobile.addEventListener('click',function(event){
                $timeout(function () {
                    $scope.show.ActiveChatOn=true;
                }, 1);
            })
        }
        ...
        var EnterSend=function(){
            ...
            for (let Enter of EnterSendSelectors) { 
                Enter.addEventListener('keydown',function(event){
                      ...
                      if(event.keyCode=='13'){
                            CreatrMessage(Enter,id,FoundIndex)
                      }
                    SendButton=document.querySelector('.SendMessButton')
                ...
                Enter.addEventListener('click',function(event){
                        id=Enter.getAttribute('chat-data-id')
                        FoundIndex=Chat.ActiveChat(id,$scope.ActiveChat)
                        if($scope.ActiveChat[FoundIndex].NotReeded>0){
                            $scope.ChatList=Chat.SetReededChat($scope.ChatList)
                            Socket.emit('MarkAsReeded',$scope.user)
                            $scope.ActiveChat=Chat.SetAllReeded(id,$scope.ActiveChat)
                            $scope.counts.NotreededMes=0;
                            document.title='StilFriend'
                        }
                })
            ...
            function CreatrMessage(Enter,id,FoundIndex){
                  if(Enter.value != ''){
                        DataSent={
        ...
        function MessagesSocket(){
            ...
            Socket.on('AddMessages',function(data){
                Socket.emit('request-messages',$scope.user)
                $scope.ChatList=Chat.Set($scope.ChatList,$scope.messages)
                if(Chat.ChatShow(data[0].UsersInConversationJason)){
                    ...
                    if(FoundIndex!=undefined){
                        if(data[0].IdSend!=$scope.user.UserID){
                            $scope.ActiveChat[FoundIndex].NotReeded=$scope.ActiveChat[FoundIndex].NotReeded+1
                            document.title='StilFriend ('+$scope.ActiveChat[FoundIndex].NotReeded+')'
                        }
        ...
        function ChatSocket(){
            ...
            Socket.on('SomeoneTypping',function(data){
                ...
                if(FoundIndex!=undefined && data.SesionData.UserID!=$scope.user.UserID){
                    $scope.ActiveChatItem.SomeoneWriting=true;
                    $scope.ActiveChatItem.TyppingPerson=data.SesionData
            ...
            Socket.on('StopTypping',function(data){
                ...
                if(FoundIndex!=undefined && data.SesionData.UserID!=$scope.user.UserID){
                   $scope.ActiveChat[FoundIndex].SomeoneWriting=false;
                    $scope.ActiveChatItem.SomeoneWriting=false;
                   $scope.ActiveChat[FoundIndex].TyppingPerson=data.SesionData
                }
        ...
        var SetUserChat=function(index,value,Mobile,Multi){ //index,value
                if(!Mobile){
                    if(Multi.length==undefined || Multi.length==0 || Multi.length==1){
                        ...
                        $scope.ActiveChat[index].NotReeded=$scope.ActiveChat[index].inChat[1].CountNotReeded
...
StartControlers.controller("Dashboard",['$scope','Url','$interval','$timeout',function($scope,Url,$interval,$timeout){
    MainDashBord()
    $timeout(function(){ 
            DashBordActions()
    },500)
    function DashBordActions(){
        AddPostSystem()
        function AddPostSystem(){
            AddPostSystem=document.querySelector('.AddPostSystemStart')
            AddPostSystem.addEventListener('focus',function(e){
                AddPostSystem.style.height=80
                AddActionsSectionOn=document.querySelector('.ActionSection')
                AddActionsSectionOn.style.display='block'
                AddActionsHideBatton=document.querySelector('.HideIcon')
                AddActionsHideBatton.style.display='block'
                AddActionsHideBatton.addEventListener('click',function(e){
                    AddPostSystemOff();
                })
            })
            function AddPostSystemOff(){
                AddPostSystem.style.height=50
                AddActionsSectionOn=document.querySelector('.ActionSection')
                AddActionsSectionOn.style.display='none'
                AddActionsHideBatton=document.querySelector('.HideIcon')
                AddActionsHideBatton.style.display='none'
            }
        }
    }

    function MainDashBord(){
            $scope.DashBordLoading=true;
            DashBordinterval=setInterval(function(){
                DashBord=Url.GetData('/GetDashBord/1')
                if(DashBord){
                    $scope.DashBordLoading=false;
                    clearInterval(DashBordinterval);
                    DashBordinterval=undefined;
                    $scope.DashBord=DashBord
                    console.log($scope.DashBord)
                }
            },25);
    }
}]);